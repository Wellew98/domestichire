...
export default function EditWorkerForm({ worker }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const skills = (formData.get("skills") as string).split(",").map((s: string) => s.trim()).filter(Boolean);
    const languages = (formData.get("languages") as string).split(",").map((s: string) => s.trim()).filter(Boolean);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium text-gray-700 mb-1">Category *</label>
            <select name="category" defaultValue={worker.category} required className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm">
              {CATEGORIES.map((c: string) => (<option key={c} value={c}>{c.replace("_"," ").replace(/\b\w/g,(l:string)=>l.toUpperCase())}</option>))}
            </select>
          </div>
